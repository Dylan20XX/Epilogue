...
public class RapidPulseArtillery extends StaticEntity {
	...
	private BufferedImage head = CT.rotateClockwise90(CT.rotateClockwise90(CT.rotateClockwise90(Assets.rapidPulse)));

	private Queue<Creatures> targets = new LinkedList<Creatures>();

	private long lastAttackTimer, attackCooldown = 120, attackTimer = 0; // attack speed every 0.5s

	private double shootAngle = Math.PI / 2;
	
	private boolean on;

	public RapidPulseArtillery(double x, double y, ControlCenter c) {
				...
				if (!target.isActive())
					...
					if (attackTimer > attackCooldown) {
						...
						c.getGameState().getWorldGenerator().getEntityManager()
								.addEntity(new RapidPulse((int)x + bounds.x + bounds.width / 2 - 10,
										(int) (y + bounds.y - 21), 45, 5, shootAngle, c));
