...
public class Flame extends Creatures {

	private double xMove;
	private double yMove;
	private double angle = 0;
	
	public Flame(int x, int y, int width, int height, double angle, ControlCenter c) {
	...
	public void tick() {
		...
		for(int i = 0; i < c.getMenuState().getWorldSelectState().getGameState().getWorldGenerator().getEntityManager().getEntitiesInBound().size(); i++) {
			...
			if(e.getBounds().intersects(getBounds()) && !e.getName().equals("player") && !e.getName().equals("pulse") && !e.equals(Player.getPlayerData()) && !e.equals(this)) {
	...
	public void render(Graphics g) {
		...
		g2d.setTransform(at);
		g2d.drawImage(Assets.flame, (int) (x - c.getGameCamera().getxOffset() - width),
