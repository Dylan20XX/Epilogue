...
public class BurntTree extends StaticEntity{
    ...
    public BurntTree(double x, double y, ControlCenter c) {
    ...
    public void render(Graphics g) {
        ...
        g.drawImage(Assets.tree1, (int)(x - c.getGameCamera().getxOffset()), (int)(y - c.getGameCamera().getyOffset()),
        		width, height, null);
	...
	public void Die() {
		...
		AudioPlayer.playAudio("audio/woodSnap.wav");
		...
		if(CT.random(1, 15) == 1) {
			c.getMenuState().getWorldSelectState().getGameState().getWorldGenerator()
			.getItemManager().addItem(Item.ropeItem.createNew((int) x + bounds.x + CT.random(0, bounds.width), (int) y + bounds.y + CT.random(0, bounds.height)));
		}
