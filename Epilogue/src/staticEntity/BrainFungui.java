...
public class BrainFungui extends StaticEntity {// java's random API
	...
	public BrainFungui(double x, double y, ControlCenter c) {
		...
		bounds.x = width/5*2;
		bounds.y = height/5*2 + height/5 + 100;
		bounds.width = width/5;
		bounds.height = height/5;

		health = 1;
		resistance = 0;
		deathImage = Assets.brainFunguiPlant;
	...
	public void Die() {
		...
		.getItemManager()
		.addItem(Food.brainFungusItem.createNew((int)(((int)x + bounds.x)*c.getScaleValue()), (int)(((int)y + bounds.y)*c.getScaleValue())));
	}
