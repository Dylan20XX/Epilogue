...
public abstract class Creatures extends Entity {
	...
	public void hurt(int amt) {
		...
		if (health <= 0) {
			...
			Die();
			c.getGameState().getWorldGenerator().addDeathAnimation(
					new DeathAnimation(c, getCurrentAnimation(), (int) (x + bounds.x + bounds.width / 2),
							(int) (y + bounds.y + bounds.height / 2), width, height, false));
	...
	public void chase (double factor) {
		...
		if (canMove) {
			...
			if (c.getMenuState().getWorldSelectState().getGameState().getWorldGenerator().getEntityManager().getPlayer()
				...
				if (x == lastX && Math.abs(velX) > Math.abs(velY) && Math.abs(velY) < speed / 4) { // creature is trying
																									// to move
																									// left/right but
																									// can't
				...
				if (y == lastY && Math.abs(velY) > Math.abs(velX) && Math.abs(velX) < speed / 4) { // creature is trying
																									// to move up/down
																									// but cant
		...
		if (canMove) {
			if (stop) {
				if (c.getMenuState().getWorldSelectState().getGameState().getWorldGenerator().getEntityManager()
						.getPlayer().getBounds().intersects(bound)) {
