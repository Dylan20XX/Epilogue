...
public class AutoCookerV2Craft {
	...
	public AutoCookerV2Craft(ControlCenter c) {
		this.c = c;
	...
	public void tick() {
		...
		if(Player.getPlayerData().isAutoCookerV2Active()){

			if (c.getKeyManager().keyJustPressed(KeyEvent.VK_E)) {
				active = false;
				Player.getPlayerData().setAutoCookerV2Active(false);
				c.getMenuState().getWorldSelectState().getGameState()
				.getWorldGenerator().setAutoCookerV2Active(false);
	...
	public void render(Graphics g) {
		...
		if (active) {
			...
			CustomTextWritter.drawString(g, "Auto Cooker V2", (int) (c.getWidth() / 2 * c.getScaleValue()),
