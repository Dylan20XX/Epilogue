...
public class AutoCookerCraft {
	...
	private ArrayList<AutoCookerRecipe> recipeList = new ArrayList<AutoCookerRecipe>(); //Stores recipes that can be crafted
	...
	private AutoCookerRecipe selectedRecipe;
	...
	public AutoCookerCraft(ControlCenter c) {
		this.c = c;
	...
	private boolean isRecipeCraftable(AutoCookerRecipe recipe) {
			...
			public void onClick() {
				...
				if (craftableItem != null) {
					...
					if (isRecipeCraftable(selectedRecipe)) {
						...
						//Exit cooking menu
						active = false;
						Player.getPlayerData().setAutoCookerActive(false);
						c.getMenuState().getWorldSelectState().getGameState()
						.getWorldGenerator().setAutoCookerActive(false);
	...
	public void render(Graphics g) {
		...
		if (active) {
			...
			CustomTextWritter.drawString(g, "Auto Cooker", (int) (c.getWidth() / 2 * c.getScaleValue()),
	...
	public long getLastCookTimer() {
		return lastCookTimer;
	}

	public void setLastCookTimer(long lastCookTimer) {
		this.lastCookTimer = lastCookTimer;
	}
